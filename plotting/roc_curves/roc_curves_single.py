...
def main():
    ...
    selected_negative_indexes, selected_positive_indexes \
        = machine_learning.get_selected_indexes(users_start_row_number, users_end_row_number, user_ids, settings.USER_ID)
    ...
    file_name = write_to_file(scores_array)
    title = 'ROC curve (AUC) user' + str(settings.USER_ID)
    computeAUC(file_name, title)

    #T1 = rndfr.predict_proba(X_validation_1)
    #print('T1:', T1)
    #print(len(T1))

    #T0 = rndfr.predict_proba(X_validation_0)
    #print('T1:', T0)
    #print(len(T0))

    #score_file_name = write_to_file(T1, T0)
    #computeAUC(score_file_name)
...
def write_to_file(prediction_results):
    ...
    a = s[index:]
    b = a[::-1]
    file_name = b + str(settings.USER_ID) + '.csv'
...
# def write_to_file(preditction_results_pos, preditction_results_neg):
#     s = settings.SCORES_OUTPUT[::-1]
#     index = s.find('_')
#     a = s[index:]
#     b = a[::-1]
...
#     with open(file_name, 'w', newline='') as csv_output:
#         data_writer = writer(csv_output, delimiter=',')
#         for i in range(0, len(preditction_results_pos)):
#             row = []
#             if preditction_results_pos[i][1] < settings.THRESHOLD:
#                 row.append(0)
#             else:
#                 row.append(1)
#             row.append(preditction_results_pos[i][1])
#             data_writer.writerow(row)
#         for i in range(0, len(preditction_results_neg)):
#             row = []
#             if preditction_results_neg[i][1] > settings.THRESHOLD:
#                 row.append(0)
#             else:
#                 row.append(1)
#             row.append(preditction_results_neg[i][1])
#             data_writer.writerow(row)
#     return file_name


def computeAUC(score_file_name, title):
