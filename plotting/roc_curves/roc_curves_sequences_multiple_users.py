...
def main(user_id):
    ...
    score_file_name = write_to_file(scores_sec_array, user_id)
    title = 'ROC curve (AUC) user' + str(user_id)
    figure_name = 'ROC_curve_sequences_user' + str(user_id) + '.png'
    computeAUC(score_file_name, title, figure_name)
...
def write_to_file(prediction_results, user_id):
    ...
    a = s[index:]
    b = a[::-1]
    file_name = b + str(user_id) + '_sequence.csv'
