...
def write_to_file(prediction_results):
    with open(settings.ALL_SCORES_OUTPUT, 'a', newline='') as csv_output:
        data_writer = writer(csv_output, delimiter=',')
        for x in prediction_results:
            data_writer.writerow(x)


def computeAUC(score_file_name):
    ...
    # plt.show()
    plt.savefig('ROC_curve_all_users.png')
    # end plot
    return auc_value
...
def main_1():
    ...
    computeAUC(settings.ALL_SCORES_OUTPUT)
