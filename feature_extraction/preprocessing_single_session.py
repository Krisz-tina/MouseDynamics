...
def get_user_name_from_file_name():
    path = settings.PATH[::-1]  # reverse path
...
def get_session_from_file_name():
    path = settings.PATH[::-1]  # reverse path
    pos = path.find('/')
    session = path[:pos]
    session = session[::-1]
...
def determine_actions():
    ...
    with open(settings.PATH, 'r') as csv_file:
        ...
        with open(settings.OUTPUT, 'w', newline='') as csv_output:
            data_writer = csv.writer(csv_output, delimiter=',')
            ...
            user_name = get_user_name_from_file_name()
            session = get_session_from_file_name()
...
def record_mouse_action(data, limit, attributes):
    ...
    if is_long_event(filtered_mouse_events):
        ...
        print(data['from'], ' ', data['to'], ' ', data['mouse_action'])
        if attributes['ds'] == 0:
            return
...
def handle_drag_and_drop(data, attributes):
    ...
    if split != 0:
        ...
        if is_long_event(filtered_mouse_events):
            ...
            determine_attributes(filtered_mouse_events, attributes, data['session'])
            print(data['from'], ' ', data['to'], ' ', data['mouse_action'], ' before DD')
            if attributes['ds'] == 0:
                return
            make_new_record(data, attributes)
            data['index_action'] += 1
    ...
    if is_long_drag_and_drop(filtered_mouse_events):
        ...
        print(data['from'], ' ', data['to'], ' ', data['mouse_action'], ' DD')
        make_new_record(data, attributes)
    ...
    for i in range(1, len(mouse_events)):
        ...
        if (float(previous_row[1]) == float(row[1])
            and previous_row[3] == row[3]) \
                or (int(previous_row[4]) == int(row[4])
                    and int(previous_row[5]) == int(row[5])
                    and previous_row[3] == row[3]) \
